...
// A file is generated when one of the headers comes before its first code, after a comment
// block such as the license header protoc-gen-go copies from a proto.
func TestIsGeneratedReadsPastALeadingComment(t *testing.T) {
	const header = "// Code generated by protoc-gen-go. DO NOT EDIT."
	dir := t.TempDir()
	for name, c := range map[string]struct {
		...
		"other":      {"// Code generated by stringer. DO NOT EDIT.\npackage p\n", false},
	} {
		path := filepath.Join(dir, name+".go")
		if err := os.WriteFile(path, []byte(c.body), 0o644); err != nil {
			t.Fatal(err)
		}
		if got := isGenerated(path, []string{header}); got != c.want {
			t.Errorf("%s: isGenerated = %v, want %v", name, got, c.want)
		}
	}
}
