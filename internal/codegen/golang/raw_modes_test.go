...
func TestParityTransportCallMatchesStubSignature(t *testing.T) {
	...
	for _, m := range svc.Methods {
		...
		td, err := buildTransportData(m, svc.Decorators(m), imps, pkg, resolverFor(pkg, nil))
		if err != nil {
			t.Fatalf("%s: %v", m.Name, err)
		}
		sd := buildServiceData(pkg.Name, "DemoService", m, svc.Decorators(m), imps, nil)
		if td.Sig != sd.Sig {
...
func generateOpenAPIToString(t *testing.T, src string) string {
	...
	if err := docs.GenerateOpenAPI(proj, sampleConfig(), root); err != nil {
		t.Fatal(err)
	}
