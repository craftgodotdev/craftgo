...
import (
	...
	"github.com/craftgodotdev/craftgo/tests/e2e/matrix/deploy/notifier/internal/transport"
...
func Register(ctx context.Context, srv *server.Server, svcCtx *svccontext.ServiceContext) (func(context.Context) error, error) {
	...
	if svcCtx.Events.Bus == nil {
		return nil, errors.New("wiring: the design declares 10 event(s) and 7 consumer(s) but svcCtx.Events carries no bus - build one in main.go and assign `svc.Events = svccontext.NewEvents(bus)`")
	}
