...
- HTTP handler at `POST /api/v1/users` (`/api` is the manifest's `openapi.basePath`)
...
The line between framework and your code is sharp. Regenerated Go files start with `// Code generated by craftgo. DO NOT EDIT.` (`openapi.yaml` with `# Generated by craftgo. DO NOT EDIT.`), scaffolds with `// Scaffold generated by craftgo. Safe to edit; will not be overwritten.` Logic stubs are scaffold-once and stay yours forever.
...
- **Codegen drift detection**: the same design always produces byte-identical output, so CI can re-run `craftgo gen` and fail on any change (`git status --porcelain`). Catches "I forgot to regen".
