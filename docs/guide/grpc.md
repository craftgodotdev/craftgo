...
`option go_package` is not needed. craftgo places every design proto's Go package under `output.pb` (default `./internal/pb`), mirroring the proto's directory: `design/greet/greet.proto` becomes `internal/pb/greet`, import path `<module>/internal/pb/greet`, Go package `greet`. A `go_package` you do write keeps its `;name` suffix for the package name; its path is ignored. Two protos in one directory must declare one package, as Go would demand of the directory, and a proto directly in the design root lands in `output.pb` itself as package `pb` - give it a directory. An RPC may not be named `Server` (the server struct's file) or `Logger` (the `log.Logger` its logic type embeds), and two RPCs may not be named `X` and `NewX` (the logic type and its constructor); each is refused before anything is written.
