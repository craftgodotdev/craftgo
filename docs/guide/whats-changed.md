...
**Drop the manifest keys that are gone.** Four are now rejected while the manifest is read, each naming what replaced it (`<key> is no longer a manifest key - <note>; drop it`): `design`, `output.services`, `output.consumeMiddleware` and `events.asyncapi`. A key the manifest shape does not declare would otherwise be ignored in silence, so the run is refused rather than generating something other than what the file says.
