---
title: What changed
description: What is new since craftgo 1.7.1 - events in the DSL, the event runtime, the generated wiring call, the manifest keys that are gone, and what to do when you upgrade.
---
...
**Drop the manifest keys that are gone.** `design`, `output.services`, `output.consumeMiddleware` and `events.asyncapi` are ignored, and `craftgo gen` warns about each, naming what replaced it (`<key> is no longer a manifest key and is ignored - <note>`), as it warns about any other key the manifest does not declare.
